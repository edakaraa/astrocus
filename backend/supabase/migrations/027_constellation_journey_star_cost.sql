-- Star unlock cost by constellation journey position (unlock_order), flat per tier.
-- First 4 constellations (unlock_order 0–3): 500 ✦/star
-- Next 4 (4–7): 1200 ✦/star
-- Remaining (8+): 2000 ✦/star

-- Parameter renamed from p_completed_count; Postgres requires drop before replace.
drop function if exists public.compute_star_cost(integer);

create or replace function public.compute_star_cost(p_unlock_order integer)
returns integer
language sql
immutable
as $$
  select case
    when p_unlock_order < 4 then 500
    when p_unlock_order < 8 then 1200
    else 2000
  end;
$$;

-- Catalog per-star values are ignored; pricing is tier-based only.
update public.stars
set required_stardust = 0
...
begin
  ...
  if v_star.constellation_id is not null then
    ...
    select uc.unlock_order
    into v_unlock_order
    from public.user_constellations uc
    where uc.user_id = v_user_id
      and uc.constellation_id = v_star.constellation_id;

    v_cost := public.compute_star_cost(coalesce(v_unlock_order, 0));
  else
    select * into v_profile from public.profiles where id = v_user_id for update;
    if not found then
      raise exception 'profile_not_found' using errcode = 'P0001';
...
grant execute on function public.compute_star_cost(integer) to authenticated;
grant execute on function public.unlock_star(text) to authenticated;
