...
> **Versiyon:** MVP v1.3
> **Durum:** Ürün kararları + uygulama revizyonu (2026-06-05)
> **Hazırlayan:** Eda Kara
...
| T-18 | Uygulama revizyonu: takımyıldızı kartları, 10 ✦/dk, manuel unlock, 4 slayt onboarding | §3.3, §4.2–§4.3, §4.10 |
| T-19 | Bilinçli olarak yapılmayacak maddeler işaretlendi (duraklatma 5 dk, skip, varsayılan yıldız, seans ekranında yıldız) | §3.3 |

---

## 3.3 Ürün Kararları — PRD'den Bilinçli Sapma (2026-06-05)

Aşağıdaki maddeler **eksik değil**; MVP hedefleri ve geliştirme maliyeti göz önüne alınarak **bilinçli ürün kararıdır**.

### Yapılmayacak (v1)

| Konu | Eski PRD | Karar | Gerekçe |
|------|----------|-------|---------|
| Duraklatma > 5 dk → seans iptali | §4.1 | **Yapılmayacak** | Ek maliyet; UX katkısı düşük; MVP için kritik değil |
| Onboarding "Atla" butonu | §4.10 | **Yapılmayacak** | 4 slayt swipe ile geçilebilir; ayrı buton değeri düşük |
| Atlayınca varsayılan yıldız | §4.10 | **Yapılmayacak** | `star-pick` zorunlu; kayıt sonrası takımyıldızı seçimi var |
| Seans ekranında seçili yıldız görseli | §4.2 | **Yapılmayacak** | Odak öncelikli UX; ekranda yalnızca süre + kategori |
| Otomatik yıldız açılması | §4.3 | **Yapılmayacak** | Manuel unlock; kullanıcı ödülü ne zaman harcayacağını seçer |

### Tasarım değişikliği (eksik sayılmaz)

| Konu | Eski PRD | Mevcut ürün |
|------|----------|-------------|
| Gökyüzü / Galaksi | Canlı gökyüzü simülasyonu, haritada yıldızlar | **Takımyıldızı kartları**; kullanıcı yıldız tozu ile sırayla açar |
| Onboarding | 3 ekran | **4 slayt** + ayrı `star-pick` rotası |
| Yıldız tozu ekonomisi | 10 ✦/dk temel + saat dilimi kategori bonusu | **10 ✦/dk**; streak + duraklatmadan tamamlama bonusu; günlük hedef +250 ✦; katalog maliyetleri ×5 (migration 024–025); saat dilimi bonusu v1.1 adayı |

### v1.1 adayı (yayın bloklayıcı değil)

- Profilde **en uzun streak** gösterimi
- Streak kırılınca motivasyon mesajı ("Yeni başlangıç")
- **Saat dilimi bonusu** — arka planda değil, görünür rozetlerle (🌙 Gece Kaşifi, ☀️ Sabah Yıldızı, 🌅 Gün Batımı Bonus)
- Geçmiş günler / haftalık geçmiş sekmesi (bugün → dün → geçen hafta)

### AI mimarisi (güvenlik)

| Akış | Katman | Sağlayıcı |
|------|--------|-----------|
| Seans sonu galaktik tavsiye | Mobil → **Express API** → LLM | Gemini (`GEMINI_API_KEY` sunucuda) |
| Haftalık AI rapor | **Supabase Edge Function** → LLM | OpenRouter (`OPENROUTER_API_KEY` Edge ortamında) |

API anahtarları **mobil bundle'da tutulmaz**.
...
| `/frontend` + `/backend` | ✅ |
| Canlı deploy | 🟡 `docs/backend-deploy.md` |
| CI pipeline | ✅ `.github/workflows/ci.yml` |
| Ürün analitiği / crash | 🟡 Kod hazır; PostHog + Sentry key'leri production'da |
| `prodocs` dokümanlar | ✅ |
