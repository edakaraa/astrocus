...
-**iOS Arka Plan Mantığı:** iOS tarafında süregelen bir arka plan sayacı (Background Tasks) kullanılamaz. Bunun yerine, uygulama arka plana alındığında zaman damgası (timestamp) kaydedilir. 10. ve 20. saniyedeki tolerans uyarıları `UNUserNotificationCenter` ile fire-at zamanlı lokal bildirim olarak zamanlanır. Uygulamaya dönüldüğünde fark hesaplanarak süre güncellenir. Ortam sesi açıksa `AVAudioSession` sessize alınarak arka plan canlı tutulabilir.
...
- **Oturum yönetimi:** **Oturum yönetimi:** JWT mimarisi. Access token (1 saat ömürlü) memory'de, Refresh token (30 gün ömürlü) cihazın güvenli katmanında (SecureStore/Keychain) saklanır.
...
| **Backend** | **Node.js (NestJS) + Prisma ORM** — Frontend ile aynı dili (TypeScript) konuşur. Modüler yapısı ve yerleşik WebSocket (Socket.io) desteği ile Global Odalar için uygundur. Stardust manipülasyonunu engellemek adına ilk sprint'te **SupabaseAuthGuard** yazılmalı; tüm korumalı endpoint'ler bu katmandan geçmelidir. |
...
| Gereklilik | Hedef |
|---|---|
| **API Response Time** | < 200ms (p95) — Supabase serverless cold start göz önünde bulundurularak; sık kullanılan endpoint'ler Upstash Redis ile önbelleğe alınarak hedef < 150ms'ye çekilebilir |
...
| **Çevrimdışı Destek** |Seanslar cihazda şifreli kuyrukta (SecureStore) tutulur. İnternet gelince NestJS sync endpoint'ine toplu iletilir. **Source of Truth:** Offline seansların `completed_at` damgası maksimum 48 saat geriye dönük olabilir. Daha eski kayıtlar hile koruması gereği reddedilir. Streak hesaplamaları sync anında retroaktif olarak yapılır. |
